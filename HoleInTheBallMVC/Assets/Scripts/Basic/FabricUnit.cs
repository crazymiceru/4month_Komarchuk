...
namespace Hole
{
    internal sealed class FabricUnit
    {
        ...
        private void MakeRunForUnit()
        {
            ...
            {
                [TypeItem.BonusHeart] = new FabricDataForUnit() { addUnitMetod = AddBonus, ObjName = "Bonus/Heart" },
                [TypeItem.BonusInv] = new FabricDataForUnit() { addUnitMetod = AddBonus, ObjName = "Bonus/Invulnerability" },
                [TypeItem.BonusPoison] = new FabricDataForUnit() { addUnitMetod = AddBonus, ObjName = "Bonus/Poison" },
                [TypeItem.Coin] = new FabricDataForUnit() { addUnitMetod = AddCoin, ObjName = "Bonus/coin" },
                ...
                [TypeItem.Player] = new FabricDataForUnit() { addUnitMetod = AddPlayer, ObjName = "Player" },
        ...
        void AddUnitComponents(IUnit unitFlag, object param = null, DataGameForSave data = null)
        {
            var (typeItem, numCfg) = unitFlag.GetTypeItem();
            _fabricRunUnit[typeItem].addUnitMetod.Invoke(unitFlag, numCfg, param, data);
        }

        public GameObject CreateUnit(TypeItem typeItem, Vector3 pos, Quaternion rot, int numCfg = 0, object param = null, DataGameForSave data = null)
        {
            ...
            (unitFlag as UnitView).SetTypeItem(cfg: numCfg);
            ...
            AddUnitComponents(unitFlag, param, data);
        ...
        void AddPlayer(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            var name = "Player";
            ...
            Reference.inst.saveController = new SaveController(unit, unitFlag as UnitView, data);
            _listControllers.Add(Reference.inst.saveController);
        }

        void AddEnemyLaser(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
        ...
        void AddEnemyBurner(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
        ...
        void AddNone(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            Debug.Log($"Unit of the { (unitFlag as MonoBehaviour).name} is given a type of None");
        }

        void AddEnemyRocketLauncher(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
        ...
        void AddEnemyRocket(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
        ...
        void AddEnemyFire(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            var name = "Enemy/Fire";
            var unit = new UnitM();
        ...
        void AddCoin(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            var name = "Coin";
            var unit = new UnitM();
            var unitData = DataObjects.inst.GetValue<UnitData>($"Data/{name} {numCfg}");
            _listControllers.Add(new UnitController(unit, unitFlag as UnitView, unitData), name);
            _listControllers.Add(new MoveController(unit, unitFlag as UnitView, unitData));
            _listControllers.Add(new RotateController(unit, (unitFlag as UnitView).transform, DataObjects.inst.GetValue<UnitRotateData>($"Data/{name}Rotate {numCfg}")));
            if (unitFlag is UnitViewTraectory)
                _listControllers.Add(new MoveTrackController(unit, unitFlag as UnitViewTraectory, DataObjects.inst.GetValue<UnitData>($"Data/{name} {numCfg}")));
            _listControllers.Add(new SaveController(unit, unitFlag as UnitView, data));
        }

        void AddBonus(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            var name = "Bonus";
            var unit = new UnitM();
            var unitData = DataObjects.inst.GetValue<UnitData>($"Data/{name} {numCfg}");
            ...
            if (unitFlag is ITraectory)
                _listControllers.Add(new MoveTrackController(unit, unitFlag as UnitViewTraectory, DataObjects.inst.GetValue<UnitData>($"Data/{name} {numCfg}")));
            if (unitData.selfGuided)
            {
                _listControllers.Add(new SelfGuidedController(unit, unitFlag as UnitView, DataObjects.inst.GetValue<UnitSelfGuidedData>($"Data/{name}SelfGuided {numCfg}")));
            }
            _listControllers.Add(new SaveController(unit, unitFlag as UnitView, data));
        }

        void AddEnvironment(IUnit unitFlag, int numCfg, object param = null, DataGameForSave data = null)
        {
            var unit = new UnitM();
